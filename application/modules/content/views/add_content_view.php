    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <div class="box box-primary">
                ...
                <!-- form start -->
			<div ng-app="validationApp" ng-controller="ContentControllerAdd">
                <form action="<?php echo site_url(). '/content/content_save'; ?>" id="form" method="post" enctype="multipart/form-data" name="contentForm"  ng-submit="submitForm()" novalidate >
							<input type="hidden" value="<?=$id?>" name="id"/>
                  <div class="box-body">
				  
						<div class="form-group" ng-class="{ 'has-error' : contentForm.mtittle.$touched  && contentForm.mtittle.$invalid }">
							<label>Judul</label>
							<input name="mtittle" placeholder="Title" class="form-control" type="text" value="<?=$mtittle?>"  ng-model="content.mtittle" required>
						...
						<div class="form-group" ng-class="{ 'has-error' :  contentForm.mcontent.$touched  && contentForm.mcontent.$invalid }">
							...
							type="text" value="<?=$mcontent?>" autocomplete="off"  
							required  >
							</textarea>
						...
						<div class="form-group" ng-class="{ 'has-error' :  contentForm.mimage.$touched  && contentForm.mimage.$invalid }" >
						  <label for="mimage">Gambar</label>
						  <input type="file" id="mimage" name="mimage" value="<?=$mimage?>" >
						  <p ng-show="contentForm.mimage.$touched  && contentForm.mimage.$invalid" class="help-block">Wajib diisi.</p> 
						</div>
						
						<input type="hidden" value="<?=$mbrowser?>" name="mbrowser"/>
						<input type="hidden" value="<?=$mipaddress?>" name="mipaddress"/>
						<input type="hidden" value="<?=$muser?>" name="muser"/>
						<input type="hidden" value="<?=$mstatus?>" name="mstatus"/>
...
// create angular app
	...
	validationApp.controller('ContentControllerAdd', function($scope) {
		//$scope.tbl_mcontent.mcontent.textRequired = true;
